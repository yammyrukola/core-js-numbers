...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorModulA = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vectorModulB = Math.sqrt(x2 ** 2 + y2 ** 2);

  return Math.acos(scalarProduct / (vectorModulA * vectorModulB));
}
...
function getSumOfDigits(num) {
  const result = [...num.toString()].reduce(
...
function isPowerOfTwo(num) {
  return Number(num.toString(2).slice(1)) === 0;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const getFractionLength = (num) => {
    const str = num.toString();
    return str.includes('.') ? str.length - str.indexOf('.') - 1 : 0;
  };

  const maxFraction = Math.max(
    getFractionLength(x1),
    getFractionLength(x2),
    getFractionLength(x3)
  );
  const sum = x1 + x2 + x3;
  return maxFraction > 0 ? Number(sum.toFixed(maxFraction)) : sum;
}
...
function getHypotenuse(a, b) {
  ...
  result = result === Infinity ? 1.7976931348623157e308 : result;
